...
API_TOKEN = '7784020444:AAFs-wBkRi0LI9PUL7ek2XOSCZ0svrO6e-o'  # Замените на токен вашего бота
...
async def send_welcome(message: types.Message):
    if message.from_user.id in banned_users:
        await message.reply("Вы забанены и не можете запустить квиз.")
        return
    logging.info(f"Command /start received from user {message.from_user.id}")  # Логирование
    user_states[message.from_user.id] = {
        "current_question": 0,
    ...
    welcome_text = (
        "добро пожаловать. ты получил этот мерч. теперь тебе осталось разгадать все его загадки. если ты сделаешь это первым — получишь ‘Ў#B±БRСр$3br. удачи"
    )
...
if __name__ == '__main__':
    asyncio.run(main())
>>>>>>> db5cf426022cc21513a5631f0eac786e3c8b4f5b
